...
class SimpleEcommCartCartItem {
  ...
  public function __construct($productId=0, $qty=1, $optionInfo='', $priceDifference=0) {
    ...
    // SimpleEcommCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] New Cart Item Option Info: $optionInfo");
  }
  ...
  public function getProduct() {
    $product = new SimpleEcommCartProduct($this->_productId);
    return $product;
  }
  ...
  public function getOptionInfo() {
    ...
    if($this->isSubscription()) {
      ...
      //SimpleEcommCartCommon::log("This is a subscription product: $options " . $this->getItemNumber());
    }
    return $options;
  }
  ...
  public function getProductPrice() {
    if($this->_productId > 0) {
      ...
      elseif($product->is_user_price == 1){
        if(SimpleEcommCartSession::get("userPrice_$this->_productId")){
          // using a user-defined price
          $userPrice = SimpleEcommCartSession::get("userPrice_$this->_productId");
  ...
  public function getProductPriceDescription() {
    if($this->_productId > 0) {
      ...
      elseif($product->isSpreedlySubscription()) {
        $product = new SimpleEcommCartProduct($product->id);
  ...
  public function isEqual(SimpleEcommCartCartItem $item) {
    $isEqual = true;
  ...
  public function detachFormEntry($lead_id) {
    ...
    SimpleEcommCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Start to detach gravity forms: " . print_r($entries, true));
    
    if(in_array($lead_id, $entries)) {
      SimpleEcommCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Lead id is in the list of entries: " . $lead_id);
